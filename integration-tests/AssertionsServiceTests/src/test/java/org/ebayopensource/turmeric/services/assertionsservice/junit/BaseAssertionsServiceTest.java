...
import org.ebayopensource.turmeric.repository.v1.services.AssertionReport;
...
import org.ebayopensource.turmeric.repository.v1.services.TextContent;
import org.ebayopensource.turmeric.repository.v1.services.URLContent;
...
public class BaseAssertionsServiceTest{
	...
	private AssertionsServiceConsumer consumer = new AssertionsServiceConsumer();
	...
	protected static String validateServiceResponse(ApplyAssertionsResponse response, String criteria) {
		...
		//Verify whether the response is in the expected way. ie. whether the assertions pass for POSITIVE_CASE and fail for NEGATIVE_CASE 
	...
	protected static String validateServiceResponse(ApplyAssertionGroupsResponse response, String criteria) {
		...
		//Verify whether the response is in the expected way. ie. whether the assertionGroups pass for POSITIVE_CASE and fail for NEGATIVE_CASE
		if(criteria.equalsIgnoreCase(POSITIVE_CASE)) {
    		if(response.getAck().value().equalsIgnoreCase(SUCCESS)) {
    				...
    				for(AssertionGroupReport assertionGroupReport : response.getAssertionGroupReport()){
    					...
    					for(ArtifactReport artifactReport : assertionGroupReport.getArtifactReport()){
    						for(AssertionResult assertionResult : artifactReport.getAssertionResults()){
    							...
    							if(assertionResult.getAssertionSeverity() == AssertionSeverity.ERROR && assertionResult.getValidationResult() != ValidationResult.PASSED){
        							return FAILURE;
        						}
        					}
    					}
    				...
    				for(AssertionGroupReport assertionGroupReport : response.getAssertionGroupReport()){
    					...
    					for(ArtifactReport artifactReport : assertionGroupReport.getArtifactReport()){
    						for(AssertionResult assertionResult : artifactReport.getAssertionResults()){
    							...
    							//WARNING assertions are not considered
    							if(assertionResult.getAssertionSeverity() == AssertionSeverity.ERROR && assertionResult.getValidationResult() == ValidationResult.FAILED){
        							return SUCCESS;
        						}
        					}
    					}
	...
	protected void displayResponse(final ApplyAssertionGroupsResponse response){
		...
		if(response != null && response.getAck().value().equalsIgnoreCase(SUCCESS)){
			...
			for(AssertionGroupReport assertionGroupReport : response.getAssertionGroupReport()){
				System.out.println("Assertion Group : " + assertionGroupReport.getAssertionGroup().getName() + " : " + assertionGroupReport.getAssertionGroup().getVersion());
