...
/*			List<Library> libraries = getLibraryListResponse.getLibrary();
			...
			System.out.println("testGetLibraryList resulted in : " + getLibraryListResponse.getAck().value());
			for (Library library : libraries) {
				System.out.println("Library name: " + library.getLibraryName() + " Library Id: " + library.getLibraryId());
			}
*/	
	}
	...
	public static String testGetLibraryList_insufficientPrivilege()
	{
		...
		try {
			...
			GetLibraryListResponse getLibraryListResponse = getLibraryListConsumer.getProxy().getLibraryList(getLibraryListRequest);			
			if(getLibraryListResponse == null) {
				throw new ServiceException(null, "Response object can not be null", null);
			}
