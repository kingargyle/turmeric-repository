...
public class UnlockAssetConsumer extends BaseRepositoryServiceConsumer {
    ...
    public static String testUnlockAsset_validAsset(AssetKey assetKey) {
        ...
        try {
            ...
            if (validateUnlockAssetResponse(unlockAssetResponse, "positiveCase").equalsIgnoreCase(
                            "success")) {
                return "PASSED";
            }
            else {
    ...
    public static String testUnlockAsset_invalidAsset() {
        ...
        try {
            ...
            if (validateUnlockAssetResponse(unlockAssetResponse, "negativeCase").equalsIgnoreCase(
                            "success")) {
    ...
    protected AsyncTurmericRSV1 getProxy() throws ServiceException {
        if (m_proxy == null) {
            ...
            Service service = ServiceFactory.create(svcAdminName,
                            RepositoryServiceClientConstants.SERVICE_NAME);
            service.setSessionTransportHeader("X-TURMERIC-SECURITY-USERID",
                            RepositoryServiceClientConstants.USER_ID);
    ...
    public static String validateUnlockAssetResponse(UnlockAssetResponse unlockAssetResponse,
                    String criteria) {
        if (criteria.equalsIgnoreCase("positiveCase")) {
            if (unlockAssetResponse.getAck().value()
                            .equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {
                return RepositoryServiceConsumerUtil.validateAssetInfo(unlockAssetResponse
                                .getAssetInfo());
            }
            if (unlockAssetResponse.getAck().value()
