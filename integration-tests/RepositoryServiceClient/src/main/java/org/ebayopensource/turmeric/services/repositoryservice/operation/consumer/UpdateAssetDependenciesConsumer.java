...
public class UpdateAssetDependenciesConsumer extends
		BaseRepositoryServiceConsumer {
	private AsyncTurmericRSV1 m_proxy = null;
	public static String testUpdateAssetDependencies_validInput(AssetInfo sourceAssetInfo,AssetInfo targetAssetInfo,AssetInfo currentTargetAssetInfo ) {
	...
	/*	RelationForUpdate relationForUpdate = new RelationForUpdate();
		relationForUpdate.setCurrentSourceAsset(null);
		relationForUpdate.setCurrentTargetAsset(null);
		Relation relation  = new Relation();
		relation.setAssetRelationship("dependent_service");
		AssetKey targetAssetKey = new AssetKey();
		targetAssetKey.setAssetId("1.0_1228437745125312141751");
		...
		try	{
			UpdateAssetDependenciesResponse updateAssetDependenciesResponse = updateAssetDependenciesConsumer.getProxy().updateAssetDependencies(updateAssetDependenciesRequest);
			if(updateAssetDependenciesResponse == null) {
				throw new ServiceException(null, "Response object can not be null", null);
			}	
			if(updateAssetDependenciesResponse.getAck().value().equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {							
	...
	public static String testUpdateAssetDependencies_deleteRelationship(AssetInfo sourceAssetInfo,AssetInfo currentTargetAssetInfo) {
		UpdateAssetDependenciesConsumer updateAssetDependenciesConsumer = new UpdateAssetDependenciesConsumer();
		UpdateAssetDependenciesRequest updateAssetDependenciesRequest = new UpdateAssetDependenciesRequest();
