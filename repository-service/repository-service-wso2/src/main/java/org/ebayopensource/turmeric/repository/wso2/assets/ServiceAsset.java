...
public class ServiceAsset implements Asset {
   ...
   public boolean createAsset() {
      try {
         service = serviceManager.newService(new QName(basicInfo.getNamespace(), basicInfo.getAssetName()));
         service.setAttribute(AssetConstants.TURMERIC_NAME, basicInfo.getAssetName());
         service.setAttribute(AssetConstants.TURMERIC_DESCRIPTION, basicInfo.getAssetDescription());
   ...
   public boolean hasDuplicates() {
      Service[] services;
      try {
         services = serviceManager.findServices(new DuplicateServiceFilter(basicInfo));
      } catch (GovernanceException e) {
         return false;
      }
