...
/**
 * This class provide the static error information like Id, domain, category etc. This Information then be used to
 * create ErrorData objects.
 * 
 * @author vajoshi
 * 
 */
public class ServiceBaseErrorDescriptor extends BaseEnum {
   ...
   public static final ServiceBaseErrorDescriptor PARAM_ORIGINAL_MESSAGE = new ServiceBaseErrorDescriptor(1,
            "PARAM_ORIGINAL_MESSAGE", "MarketPlaceCommon", ErrorSeverity.ERROR, ErrorCategory.REQUEST,
            "Original message");

   public static final ServiceBaseErrorDescriptor INVALID_INPUT_DATA = new ServiceBaseErrorDescriptor(2,
            "INVALID_INPUT_DATA", "MarketPlaceCommon", ErrorSeverity.ERROR, ErrorCategory.REQUEST, "Invalid input data");

   protected ServiceBaseErrorDescriptor(long errorId, String errorName, String subdomain, ErrorSeverity errorSeverity,
   ...
   public ErrorData newError() {
      return getErrorData(null);
   }

   public ErrorData newError(ErrorParameter parameter) {
      ErrorData error = getErrorData(getKeys(parameter));
      if (parameter != null) {
   ...
   public ErrorData newError(List<ErrorParameter> parameters) {
      ErrorData error = getErrorData(getKeys(parameters));
   ...
   private ErrorData getErrorData(String[] params) {
