...
/**
 * AssertionProcessorContext manages content for one or more AssertionProcessors.
 * <p>
 * The context cache consolidates multiple references to the same content and allows different AssertionProcessors to
 * share the same content. For instance the same Assertion may be in multiple groups. In that case the content source
 * for the assertion script is shared by those groups. Also many assertions may reference the same AssertionModules. The
 * module script contents need to be retrieved only once.
 * <p>
 * The context optimizes access to repository Assets. The artifact contents of an Asset are retrieved once the first
 * time an artifact for the Asset.
 * 
 * @author pcopeland
 */
public class AssertionProcessorContext {
   ...
   /** The Constant logger. */
   private static final Logger logger = Logger.getLogger(AssertionProcessorContext.class);

   /** The unique id. */
   private int uniqueId = 0; // used for keys to String and byte content

   /*
    * The cache objects aim to duplicate trips to the Repository for the same Asset. The assumption is that Assertions
    * and Module scripts are relatively small. Input content that assertions are applied against might be large but
    * there will be only a a few in each call to the service (usually only one).
    * 
    * contentCache holds artifact content as well as assertion scripts and module scripts. Input content is dereferenced
    * by AssetContentSource. Modules are usually dereferenced when their containing Assertion is loaded (lookup in
    * assetIdCache).
    * 
    * Assertions and AssertionGroups are input as AssetReferences (lookup in assetRefCache) or can be found an members
    * of AssertionGroups (lookup in assetIdCache).
    */
   /** The content cache. */
   private final Map<String, AssertionContentSource> contentCache = new HashMap<String, AssertionContentSource>();

   /** The asset ref cache. */
   private final Map<AssetReference, AssetInfo> assetRefCache = new HashMap<AssetReference, AssetInfo>();

   /** The asset id cache. */
   private final Map<String, AssetInfo> assetIdCache = new HashMap<String, AssetInfo>();

   /** The module cache. */
   private final Map<String, AssertionModule> moduleCache = new HashMap<String, AssertionModule>();

   /** The assertable cache. */
   private final Map<String, Assertable> assertableCache = new HashMap<String, Assertable>();
   ...
   /**
    * Constructs a AssertionProcessorContext without credentials for the RepositoryService.
    */
   public AssertionProcessorContext() {
   }

   /**
    * Constructs a AssertionProcessorContext with credentials for the RepositoryService.
    * 
    * @param userid
    ...
    */
   public AssertionProcessorContext(String userid, String password, String serviceLocation) {
      logger.debug("AssertionProcessorContext: userid=" + userid);
      logger.debug("AssertionProcessorContext: password=" + password);
      logger.debug("AssertionProcessorContext: serviceLocation=" + serviceLocation);
      logger.debug("AssertionProcessorContext 3 param constructor: s_cacheManager =" + s_cacheManager);
      Properties properties = new Properties();

      if (serviceLocation != null) {
         properties.setProperty(AssetInfoCapsule.RS_LOCATION, serviceLocation);
      }
      if (userid != null) {
      ...
      logger.debug("[JEGAN] AssertionFramework AssertionProcessorContext constructor username: " + userid
               + " password: " + password);
      if (s_cacheManager == null) {
         try {
            ...
            logger.debug("AssertionProcessorContext: instanciated s_cacheManager=" + s_cacheManager);
         } catch (IllegalAccessException illegalAccessException) {
            throw new RuntimeException("Could not create cacheManager due to the following reasons: "
      ...
      /*
       * try { rsClient = new RepositoryServiceClient(userid, password, serviceLocation); } catch (ServiceException se)
       * { throw new RuntimeException("Could not construct RepositoryServiceClient", se); }
       */
   }

   /**
    * Returns an AssertionContentSource for a given AssertionContent. The returned AssertionContentSource is cached for
    * subsequent calls with the same AssertionContent.
    * 
    * @param assertionContent
    *           the assertion content
    * @return an AssertionContentSource for a given AssertionContent.
    */
   public AssertionContentSource getAssertionContentSource(AssertionContent assertionContent) {
      AssertionContentSource acs = contentCache.get(assertionContent.getSourceName());
      if (acs == null) {
   ...
   /**
    * Caches an AssertionContent and the associated AssertionContentSource. If an AssertionContent with the same name is
    * already cached it is replaced with the given AssertionContent.
    * 
    * @param assertionContent
    ...
    */
   public AssetInfo getAssetInfo(AssetReferent referent) throws AssertionIllegalArgumentException {
      ...
      if (assetInfo == null) {
         ...
         if (assetInfo == null) {
            throw new IllegalArgumentException("Could not get Asset for " + assetRef);
         }
         assetRefCache.put(referent.getAssetReference(), assetInfo);
         assetIdCache.put(getPseudoAssetId(assetInfo.getBasicAssetInfo().getAssetKey()), assetInfo);
      }
      return assetInfo;
   }
    ...
    *            the assertion illegal argument exception
    */
   public Assertion getAssertion(AssetKey assetKey) throws AssertionIllegalArgumentException {
      String assetId = getPseudoAssetId(assetKey);
      AssetAssertion assertion = (AssetAssertion) assertableCache.get(assetId);
      if (assertion == null) {
         AssetInfo assetInfo = getAssetInfo(assetKey);
   ...
   public AssertionGroup getAssertionGroup(AssetKey assetKey) {
      String assetId = getPseudoAssetId(assetKey);
      AssetAssertionGroup group = (AssetAssertionGroup) assertableCache.get(assetId);
      if (group == null) {
         AssetInfo assetInfo = getAssetInfo(assetKey);
   ...
   private AssetInfo getAssetInfo(AssetKey assetKey) {
      ...
      if (assetInfo == null) {
         ...
         if (assetInfo == null) {
            throw new IllegalArgumentException("Could not get Asset for AssetId=" + assetKey.getAssetId()
                     + " AssetName=" + assetKey.getAssetName());
         }
   ...
   private AssetReference getAssetReference(AssetInfo assetInfo) {
      ...
      return new AssetReference(assetKey.getAssetId(), assetName, basicAssetInfo.getVersion(),
    ...
    */
   private AssertionContentSource newAssertionContentSource(AssertionContent assertionContent) {
      ...
      if (assertionContent instanceof AssetContent) {
         ...
         acs = new ClassResourceContentSource((ClassResourceContent) assertionContent);
      } else {
         throw new IllegalArgumentException("Unknown AssertionContent type=" + assertionContent.getClass().getName());
      }
      return acs;
   }
   ...
   private AssertionModule newAssetAssertionModule(AssetInfo assetInfo) {
      ...
      if (processorType == null) {
         throw new IllegalArgumentException("Cannot find 'assertion-processor' for Asset="
      ...
      for (ArtifactInfo artifactInfo : artifacts) {
         scriptContents
                  .add(new AssetContent(artifactInfo.getArtifact().getArtifactName(), getAssetReference(assetInfo)));
      }
      return new BasicAssertionModule(basicAssetInfo.getAssetName(), basicAssetInfo.getVersion(),
               basicAssetInfo.getAssetDescription(), scriptContents, processorType);
   }
   ...
   private AssetInfo getRespositoryAssetInfo(AssetReference assetRef) {
      /*
       * return rsClient.getAssetInfo( assetRef.getLibrary(), assetRef.getAssetName(), assetRef.getVersion(),
       * assetRef.getAssetType());
       */

      try {
         logger.debug("AssertionProcessorContext: s_cacheManager=" + s_cacheManager);
         return (AssetInfo) s_cacheManager.getAsset(assetRef.getAssetId(), assetRef.getAssetName(),
                  assetRef.getVersion(), assetRef.getAssetType());
   ...
   private AssetInfo getRespositoryAssetInfo(AssetKey assetKey) {
      if (assetKey.getAssetId() == null) {
         logger.error("AssetId should be provided **************************");
      }
      try {
         return (AssetInfo) s_cacheManager.getAsset(assetKey.getAssetId());
      } catch (AssetRetrivalException e) {
         logger.error(e.getMessage());
         return null;
      }
   ...
   public void removeAssetFromCache(String libraryName, String assetName, String assetVersion) {
      if (s_cacheManager != null) {
         s_cacheManager.removeAssetFromCache(libraryName, assetName, assetVersion);
      }
   }
