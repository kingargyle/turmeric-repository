 ...
 * @author pcopeland
 */
public class AssetContentSource extends AbstractContentSource implements AssetReferent {
   private final AssetContent assetContent;
   private final AssertionProcessorContext context;
    ...
    * 
    * @param assetContent
    *           the underlying AssertionContent.
    * @param context
    *           the context
    */
   public AssetContentSource(AssetContent assetContent, AssertionProcessorContext context) {
      super(assetContent);
   ...
   /**
    * Returns an InputStream for this AssertionContent or null if the content should be read with a character Reader.
    * 
   ...
   public InputStream getContentStream() throws IOException {
      if (inputStream == null) {
         dereference(context);
         if (content == null) {
            throw new IllegalStateException("Could not find artifact content for " + this);
         }
   ...
   /**
    * Returns a character stream reader for this AssertionContent or null if the content is not character data.
    * 
      ...
      for (ArtifactInfo artifactInfo : artifacts) {
         ...
         if (!assetInfo.getBasicAssetInfo().getAssetType().equals("AssertionModule")) {
            if (artifact != null && artifact.getArtifactCategory() != null
                     && artifactName.equalsIgnoreCase(artifact.getArtifactCategory())) {
               content = artifactInfo.getArtifactDetail();
               assetContent.setArtifactDisplayName(artifactInfo.getArtifact().getArtifactName());
            }
            if (content == null) {
               if (artifact == null) {
                  logger.error("CASE1: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' as the artifact object is null");
               } else if (artifact.getArtifactCategory() == null) {
                  logger.error("CASE1: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' as the artifact category is null");
               } else if (!artifactName.equalsIgnoreCase(artifact.getArtifactCategory())) {
                  logger.error("CASE1: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' as artifact name " + artifactName + " != " + artifact.getArtifactCategory());
               } else {
                  logger.error("CASE1: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' artifact " + artifactInfo.getArtifact().getArtifactName() + " with artifact detail: "
            ...
            if (content == null) {
               if (!artifactName.equals(artifact.getArtifactName())) {
                  logger.error("CASE2: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' as artifact name " + artifactName + " != " + artifact.getArtifactName());
               } else {
                  logger.error("CASE2: Content is NULL for asset '" + assetInfo.getBasicAssetInfo().getAssetName()
                           + "' artifact " + artifactInfo.getArtifact().getArtifactName() + " with artifact detail: "
                           + artifactInfo.getArtifactDetail() + " *********");
