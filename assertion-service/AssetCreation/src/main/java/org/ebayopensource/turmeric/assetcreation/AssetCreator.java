...
public class AssetCreator implements AssetCreatorIntf {
    ...
    * (non-Javadoc)
    * 
    * @see org.ebayopensource.turmeric.assetcreation.AssetCreatorIntf#getAssetInfoForUpdate(org.ebayopensource.turmeric.
    * repository.v1.services.AssetInfo)
    */
   @Override
    ...
    * 
    * @see
    * org.ebayopensource.turmeric.assetcreation.AssetCreatorIntf#updateCompleteAsset(org.ebayopensource.turmeric.repository
    * .v1.services.AssetInfoForUpdate)
    */
   @Override
   public UpdateCompleteAssetResponse updateCompleteAsset(AssetInfoForUpdate assetInfoForUpdate)
    ...
    * 
    * @see
    * org.ebayopensource.turmeric.assetcreation.AssetCreatorIntf#lockAsset(org.ebayopensource.turmeric.repository.v1
    * .services.AssetKey)
    */
   @Override
    ...
    * 
    * @see
    * org.ebayopensource.turmeric.assetcreation.AssetCreatorIntf#unlockAsset(org.ebayopensource.turmeric.repository.
    * v1.services.AssetKey)
    */
   @Override
    ...
    * 
    * @see
    * org.ebayopensource.turmeric.assetcreation.AssetCreatorIntf#removeAsset(org.ebayopensource.turmeric.repository.
    * v1.services.AssetKey)
    */
   @Override
            ...
            if (assetIdWrapper.isConsumed()) {
               ...
               try {
                  ...
                  if (!unlockAssetResp.getAck().value().equals(AssetCreationConstants.SUCCESS)) {
                     s_logger.error("Following errors occured while unlocking the asset: "
                              + assetIdWrapper.getAssetId());
               ...
               if (isApprovalRequired == null || isApprovalRequired.equals(AssetCreationConstants.FALSE)) {
                  ...
                  try {
                     UpdateAssetAttributesResponse updateAssetAttrRes = rsConsumer
                              .updateAssetAttributes(updateAssetAttrReq);
                     if (!updateAssetAttrRes.getAck().value().equals(AssetCreationConstants.SUCCESS)) {
                        s_logger.error("Following errors occured while updating the asset for isApprovedClassifier: "
                                 + assetIdWrapper.getAssetId());
               ...
               try {
                  ...
                  if (!lockAssetResp.getAck().value().equals(AssetCreationConstants.SUCCESS)) {
                     s_logger.error("Following errors occured while locking the asset under logidex_adm  : "
                              + assetIdWrapper.getAssetId());
         ...
         for (AssetInfoWrapper assetInfoWrapper : listOfAssetInfoWrapper) {
            ...
            if (assetInfoWrapper.isConsumed()) {
               ...
               try {
                  ...
                  if (!unlockAssetResp.getAck().value().equals(AssetCreationConstants.SUCCESS)) {
                     s_logger.error("Following errors occured while unlocking the asset: "
                              + assetInfoWrapper.getAssetInfo().getBasicAssetInfo().getAssetKey().getAssetId());
                     List<CommonErrorData> error = unlockAssetResp.getErrorMessage().getError();
                     for (CommonErrorData errorData : error) {
                        s_logger.error("ErrorId: " + errorData.getErrorId());
                        s_logger.error("Error Message: " + errorData.getMessage());
               ...
               try {
                  RemoveAssetResponse removeAssetResp = rsConsumer.removeAsset(removeAssetReq);
                  if (!removeAssetResp.getAck().value().equals(AssetCreationConstants.SUCCESS)) {
                     s_logger.error("Following errors occured while deleting the asset: "
                              + assetInfoWrapper.getAssetInfo().getBasicAssetInfo().getAssetKey().getAssetId());
                     List<CommonErrorData> error = removeAssetResp.getErrorMessage().getError();
                     for (CommonErrorData errorData : error) {
                        s_logger.error("ErrorId: " + errorData.getErrorId());
