...
public class AssetGenerator {
    ...
    */
   public static AssetStore createAsset(String configFilePath) throws AssetCreationException {
      ...
      for (AssetInput assetInput : assetInputs) {
         ...
         if (numOfAssetsPresent > 0) {
            if (assetInput.isNeedFullAsset()) {
               ...
               for (File existingAsset : existingAssets) {
                  if (!existingAsset.getName().equals(sourceAssetId + ".xml")
                           && existingAsset.getName().endsWith(".xml")) {
                     s_logger.debug("Unmarshalling persisted assetInfo for " + id + "from"
                              + existingAsset.getAbsolutePath());
                     ...
                     s_logger.debug("Cached assetId = "
                              + existingAssetInfo.getBasicAssetInfo().getAssetKey().getAssetId());
                     s_logger.debug("Cached assetName = "
                              + existingAssetInfo.getBasicAssetInfo().getAssetKey().getAssetName());
         ...
         for (int index = 1; index <= numOfAssetsNeeded; index++) {
            try {
               ...
               s_logger.debug("Asset Created... AssetId = " + assetKey.getAssetId() + " AssetName = "
                        + assetKey.getAssetName() + " for ID " + id);
               if (assetInput.isPublished() == null || assetInput.isPublished()) {
                  ...
                  s_logger.debug("Asset Published... AssetId = " + assetKey.getAssetId() + " AssetName = "
                           + assetKey.getAssetName() + " for ID " + id);
               }
               if (assetInput.isNeedFullAsset()) {
    ...
    *            the asset creation exception
    */
   public static Map<String, AssetInfo> getSourceAsset(AssetCreation assetCreation) throws ServiceException,
      ...
      try {
         ...
         for (AssetInput assetInput : assetInputs) {
            ...
            if (FileUtil.isFileExist(assetDestinationLocation, id, assetId)) {
               ...
               sourceAssetInfo = CommonUtil.unMarshalAssetInfo(assetDestinationLocation + "/" + id + "/" + assetId
                        + ".xml");
            } else {
