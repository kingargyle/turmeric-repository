...
public class Validator {
	...
	public static void validateCreateAndSubmitAssetResponse(CreateAndSubmitAssetResponse submitAssetResponse) throws AssetCreationException
	{
		...
		{
			if((submitAssetResponse.getErrorMessage().getError().size()==1 && 
					submitAssetResponse.getErrorMessage().getError().get(0).getErrorId()==109))
			{
				s_logger.warn("Creation got success but submission got " +
						"failed for assetName = "+submitAssetResponse.getAssetKey().getAssetName() 
						+" for assetId = "+submitAssetResponse.getAssetKey().getAssetId());
				return;
			}
	...
	public static void validateSubmitForPublishingResponse(SubmitForPublishingResponse submitForPubRes) throws AssetCreationException
	{
		...
		if(submitForPubRes.getAck().value().equalsIgnoreCase("success"))
		{
			if(submitForPubRes.getErrorMessages()!=null)
			{
				...
				throw new AssetCreationException("Submission got failured with error message "+submitForPubRes.getErrorMessages());
			}
			else
			{
