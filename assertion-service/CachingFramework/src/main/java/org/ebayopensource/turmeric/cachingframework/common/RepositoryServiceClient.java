...
/**
 * RepositoryServiceClient is a client for the RepositoryService web service.
 * The Assertions framework uses RepositoryService to obtain Asset artifacts
 * from the repository.
 * 
 * @author pcopeland
 */
public class RepositoryServiceClient {
	static private final Logger logger = Logger
			.getLogger(RepositoryServiceClient.class);
	...
	public RepositoryServiceClient(String userId, String password,
			String serviceLocation) throws ServiceException {
		this.userId = userId;
		this.password = password;
		try {
			URL serviceLocationURL = new URL(serviceLocation);
			this.serviceLocationURL = serviceLocationURL;
			// service = ServiceFactory.create("RepositoryService",
			// "RepositoryService", serviceLocationURL);
	...
	private TurmericRSV1 getServiceProxy() throws ServiceException {
		...
		Service service = ServiceFactory.create("TurmericRSV2",
				"TurmericRSV2", serviceLocationURL);
		...
		if (securityCookie != null) {
			...
			requestContext.setTransportHeader("X-TURMERIC-SECURITY-COOKIE",
					securityCookie);
		} else {
	 ...
	 */
	public AssetInfo getAssetInfo(AssetKey assetKey)
	 ...
	 * @return a repository Asset.
	 */
	public AssetInfo getAssetInfo(String libraryName, String assetName,
			String assetVersion, String assetType)
			throws AssertionIllegalArgumentException {
		GetAssetInfoRequest assetRequest = new GetAssetInfoRequest();
		...
		if (library == null)
			throw new IllegalArgumentException("Could not find Library named '"
					+ libraryName + "'");
	...
	private AssetInfo getAssetInfo(GetAssetInfoRequest assetRequest)
			throws AssertionIllegalArgumentException {
		String assetName = assetRequest.getAssetKey().getAssetName();
		String assetVersion = assetRequest.getVersion();
		String libraryName = assetRequest.getAssetKey().getLibrary()
				.getLibraryName();
		try {
			logger.error("RepositoryServiceClient.getAssetInfo: getting the Asset info for assetName="+assetName+", libraryName="+libraryName+",assetVersion="+assetVersion);
			GetAssetInfoResponse rsp = getServiceProxy().getAssetInfo(
					assetRequest);
			logger.error("RepositoryServiceClient.getAssetInfo: rsp="+rsp+", rsp.getAck="+rsp.getAck()+",rsp.getErrorMessage()="+rsp.getErrorMessage());
			
			if (rsp.getAck() != null && rsp.getAck().value() != null
					&& rsp.getAck().value().equalsIgnoreCase("success")) {
				if (rsp.getAssetInfo() == null) {
					throw new AssertionIllegalArgumentException(
							rsp.getErrorMessage());
				}
				...
				StringBuffer errbuf = new StringBuffer(
						"RepositoryService.getAssetinfo() failed");
				errbuf.append(" asset=").append(assetName).append(",version=")
						.append(assetVersion).append(",library=")
						.append(libraryName).append(",errors=");
				throw new AssertionRuntimeException(rsp.getErrorMessage());
				/*
				 * throw new RuntimeException(formatError( errbuf,
				 * rsp.getErrorMessage()));
				 */
			}
		} catch (ServiceInvocationRuntimeException sifex) {
			throw new RuntimeException(
					"RepositoryService.getAssetinfo() failed for" + " asset="
							+ assetName + ",version=" + assetVersion
							+ ",library=" + libraryName, sifex);
		} catch (ServiceException serviceException) {
			throw new RuntimeException(
					"RepositoryService.getAssetinfo() failed for" + " asset="
							+ assetName + ",version=" + assetVersion
							+ ",library=" + libraryName, serviceException);
		}
	}
	...
	public List<Library> getLibraryList() {
		...
		try {
			...
			logger.error("RepositoryService.getLibraryList() failed for the following reason: "
					+ msg);
			throw new RuntimeException(
					"RepositoryService.getLibraryList() failed for the following reason: "
							+ msg, serviceException);
		}
		if (rsp.getAck().value().equalsIgnoreCase("success")) {
			libraryList = rsp.getLibrary();
		} else {
			throw new RuntimeException(formatError(new StringBuffer(
