...
public class ValidateArtifactOperationImpl {
   ...
   public boolean validate(ValidateArtifactRequest validateArtifactRequest, List<CommonErrorData> errorDataList) {
      ...
      if (validateArtifactRequest == null) {
         ...
         errorDataList.add(ErrorDataFactory
                  .createErrorData(ErrorConstants.MISSING_ARTIFACT, ErrorConstants.ERRORDOMAIN));
      } else if (validateArtifactRequest.getArtifactContent().getContentType() == null) {
         isValid = false;
         errorDataList.add(ErrorDataFactory.createErrorData(ErrorConstants.ARTIFACT_CATEGORY_MISSING,
                  ErrorConstants.ERRORDOMAIN));
      } else if (validateArtifactRequest.getArtifactContent().getBinaryContent() == null
               && validateArtifactRequest.getArtifactContent().getTextContent() == null) {
         // Currently only BinaryContent and TextContent is supported to specify the artifact. UrlContent is not
         // supported.
         isValid = false;
         errorDataList.add(ErrorDataFactory
                  .createErrorData(ErrorConstants.MISSING_ARTIFACT, ErrorConstants.ERRORDOMAIN));
   ...
   public ValidateArtifactResponse validateArtifact(ValidateArtifactRequest validateArtifactRequest) {
      ...
      try {
         ...
         ApplyAssertionGroupsResponse applyAssertionGroupsResponse = s_ApplyAssertionGroupsOperationImpl
                  .applyAssertionGroups(applyAssertionGroupsRequest);
   ...
   private ApplyAssertionGroupsRequest getApplyAssertionGroupsRequest(ValidateArtifactRequest validateArtifactRequest)
      ...
      if (!setAssertionGroupAndVersion(validateArtifactRequest, assertionGroupAsset)) {
         throw new InvalidInputException(
                  "Could not find ArtifactType or assertionGroup to be applied for the ArtifactType");
      }
   ...
   private boolean setAssertionGroupAndVersion(ValidateArtifactRequest validateArtifactRequest,
            AssertionGroupAsset assertionGroupAsset) {
      ...
      switch (artifactType) {
         case WSDL:
            assertionGroupAsset.setAssetName(ConstantPopulator.getInstance().getValueOf(
                     PropertyKeys.WSDL_ASSERTION_GROUP));
            assertionGroupAsset.setVersion(ConstantPopulator.getInstance().getValueOf(
                     PropertyKeys.WSDL_ASSERTION_GROUP_VERSION));
            isSuccess = true;
            break;

         case XSD:
            assertionGroupAsset.setAssetName(ConstantPopulator.getInstance().getValueOf(
                     PropertyKeys.XSD_ASSERTION_GROUP));
            assertionGroupAsset.setVersion(ConstantPopulator.getInstance().getValueOf(
                     PropertyKeys.XSD_ASSERTION_GROUP_VERSION));
            isSuccess = true;
            break;
   ...
   private void setResponseWithValidationResult(ApplyAssertionGroupsResponse applyAssertionGroupsResponse,
            ValidateArtifactResponse validateArtifactResponse) {
   ...
   private void propagateErrors(ApplyAssertionGroupsResponse applyAssertionGroupsResponse,
            ValidateArtifactResponse validateArtifactResponse) {
      if (applyAssertionGroupsResponse != null && applyAssertionGroupsResponse.getErrorMessage() != null) {
         List<CommonErrorData> errors = applyAssertionGroupsResponse.getErrorMessage().getError();
