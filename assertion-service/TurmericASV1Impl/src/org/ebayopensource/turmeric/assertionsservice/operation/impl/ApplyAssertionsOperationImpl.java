...
public class ApplyAssertionsOperationImpl extends BaseOperationImpl
{
	...
	public boolean validate(ApplyAssertionsRequest applyAssertionsRequest, List<CommonErrorData> errorDataList)
	{
		...
		if(applyAssertionsRequest!=null)
		{
			...
			{
				isValid = false;
				errorDataList.add(ErrorDataFactory.createErrorData(ErrorConstants.ASSERTIONS_MISSING, ErrorConstants.ERRORDOMAIN));
			}
			else
			{
		...
		//Create AssertionContent
		 ...
		 for(Assertion singleAssertion: assertionList)
		 {
			...
			if(singleAssertion.getAssertionAsset()!= null)
			{
				...
				org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference apiAssetReference = new org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference(singleAssertion.getAssertionAsset().getLibraryName(),singleAssertion.getAssertionAsset().getAssetName(),singleAssertion.getAssertionAsset().getVersion(),singleAssertion.getAssertionAsset().getAssetType().value());
			...
			else if(singleAssertion.getExternalAssertion()!= null)
			{
				...
				try 
				{
					apiAssertionContent = (org.ebayopensource.turmeric.repositorymanager.assertions.AssertionContent) convert(singleAssertion.getExternalAssertion().getAssertionScript());
				} 
				...
				{
					List<AssertionModule> wsdlAssertionModuleList = singleAssertion.getExternalAssertion().getAssertionModules();
					...
					for(AssertionModule assertionModule : wsdlAssertionModuleList)
					{
						try 
						{
							apiAssertionModuleList.add((org.ebayopensource.turmeric.repositorymanager.assertions.AssertionModule) convert(assertionModule));
						} 
						catch (ObjectNotFoundException objectNotFoundException) 
						{
							isPartial = true;
				...
				apiAssertion 
				 	= new BasicAssertion(singleAssertion.getExternalAssertion().getName(),singleAssertion.getExternalAssertion().getVersion(), 
				 			singleAssertion.getExternalAssertion().getDescription(),ValidationResult.valueOf(singleAssertion.getExternalAssertion().getErrorSeverity().value()),
				 	apiAssertionContent, singleAssertion.getExternalAssertion().getAssertionProcessorType(), apiAssertionModuleList);
	     ...
	     org.ebayopensource.turmeric.repository.v1.services.AssertionReport wsdlAssertionReport = new org.ebayopensource.turmeric.repository.v1.services.AssertionReport();
	     for(AssertableArtifact assertableArtifact : assertableArtifactList)
		 {
	    	 ...
	    	 try 
	 		{
	    		...
	    		singleContentList.add((org.ebayopensource.turmeric.repositorymanager.assertions.AssertionContent)convert(assertableArtifact));
				...
				apiAssertionReport = assertionManager.applyAssertions(userId,password, apiAssertionList,singleContentList);
			...
			if(assertableArtifact!= null)
			{
			    ...
			    convert(assertableArtifact,assertableArtifactReference);
	    		 ...
	    		 //artifactReport.setArtif ValidationResult(AssertionErrorSeverity.valueOf(apiAssertionReport.getValidationResult().toString()));
	    		 for(AssertionResult assertionResult : apiAssertionReport.getAssertionResults())
	    		 {
	    			 ...
	    			 org.ebayopensource.turmeric.repository.v1.services.AssertionResult wsdlAssertionResult = (org.ebayopensource.turmeric.repository.v1.services.AssertionResult) convert(assertionResult);
	    				for(org.ebayopensource.turmeric.repositorymanager.assertions.Assertion apiAssertion : apiAssertionList)
	    				{
	    					...
	    					{
	    						wsdlAssertionResult.setAssertionSeverity(AssertionSeverity.valueOf(apiAssertion.getErrorSeverity().name()));
	    					}
	    				}
	 		...
	 		catch (IOException ioException) 
	 		{
				...
				isError = true;
				errorDataList.add(AssertionsServiceOperationUtil
						.parseGeneralExceptionMessage(runtimeException));
			}
	     }
