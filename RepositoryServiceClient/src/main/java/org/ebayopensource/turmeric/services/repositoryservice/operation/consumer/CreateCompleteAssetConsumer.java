...
/*		attributeNameValue = new AttributeNameValue();
		...
		createCompleteAssetRequest.setCaptureTemplateName(RepositoryServiceClientConstants.VALID_SERVICE_CAPTURE_TEMPLATE);
		
		try	{
			CreateCompleteAssetResponse createCompleteAssetResponse = createCompleteAssetConsumer.getProxy().createCompleteAsset(createCompleteAssetRequest);		

			if(createCompleteAssetResponse == null) {
				throw new ServiceException(null, "Response object can not be null", null);
			}	
			if(validateCreateCompleteAssetResponse(createCompleteAssetResponse, "positiveCase").equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {							
	...
	public static String testCreateCompleteAsset_invalidInput(String variant, AssetInfo targetAssetInfo) {
		...
		try	{
			...
			if(validateCreateCompleteAssetResponse(createCompleteAssetResponse, "negativeCase").equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {							
				return RepositoryServiceClientConstants.PASS;
			}
			else {
				return RepositoryServiceClientConstants.FAIL;
	...
	private static String validateCreateCompleteAssetResponse(CreateCompleteAssetResponse createCompleteAssetResponse, String criteria) {
