...
public class GetAllAssetsGroupedByCategoryConsumer extends BaseRepositoryServiceConsumer {
	...
	public static String testGetAllAssetsGroupedByCategory_withCategorizingClassifer() {
		...
		try	{
			GetAllAssetsGroupedByCategoryResponse getAllAssetsGroupedByCategoryResponse = consumer.getProxy().getAllAssetsGroupedByCategory(getAllAssetsGroupedByCategoryRequest);
			if(getAllAssetsGroupedByCategoryResponse == null) {
				throw new ServiceException(null, "Response object can not be null", null);
			}	
			if(validateGetAllAssetsGroupedByCategoryResponse(getAllAssetsGroupedByCategoryResponse, "positiveCase").equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {							
	...
	public static String testGetAllAssetsGroupedByCategory_withInvalidCategorizingClassifer() {
		...
		try	{
			...
			if(validateGetAllAssetsGroupedByCategoryResponse(getAllAssetsGroupedByCategoryResponse, "Warning").equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {							
				return RepositoryServiceClientConstants.PASS;
			}
			else {
				return RepositoryServiceClientConstants.FAIL;
    ...
    private static String validateGetAllAssetsGroupedByCategoryResponse(GetAllAssetsGroupedByCategoryResponse getAllAssetsGroupedByCategoryResponse, String criteria) {
    	    	
    	if(criteria.equalsIgnoreCase("positiveCase")) {
    		if(getAllAssetsGroupedByCategoryResponse.getAck().value().equalsIgnoreCase(RepositoryServiceClientConstants.SUCCESS)) {
				List<AssetsGroupedByCategory> assetsByCategory = getAllAssetsGroupedByCategoryResponse.getAssetsGroupedByCategory();
    	...
    	if(criteria.equalsIgnoreCase("negativeCase")) {
    		if(getAllAssetsGroupedByCategoryResponse.getAck().value().equalsIgnoreCase(RepositoryServiceClientConstants.FAILURE)) {
    			if(getAllAssetsGroupedByCategoryResponse.getErrorMessage().getError().size() > 0) {
    				return RepositoryServiceClientConstants.SUCCESS;
    			}
    		}   		
