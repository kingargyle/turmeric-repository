...
public class ApplyAssertionGroupsOperationImpl extends BaseOperationImpl
{
	...
	{
	    if (applyAssertionGroupsRequest.getArtifacts() != null && applyAssertionGroupsRequest.getArtifacts().size() > 0)
	    {
		for (AssertableArtifact assertableArtifact : applyAssertionGroupsRequest.getArtifacts())
		{
		    ...
		    if(assertableArtifact.getArtifactAssetReference()!= null && !ArtifactContentTypes.WSDL.equals(assertableArtifact.getArtifactAssetReference().getArtifactCategory()))
	    	{
		    	isAllWSDL = false;
    		}
		    else if(assertableArtifact.getArtifactExternal() != null && !ArtifactContentTypes.WSDL.equals(assertableArtifact.getArtifactExternal().getContentType()))
		    {
		    	isAllWSDL = false;
		    }
		}
	    ...
	    if (applyAssertionGroupsRequest.getAssertionGroups() != null && applyAssertionGroupsRequest.getAssertionGroups().size() > 0)
	    {
	    ...
	    else
	    {
	    	...
	    	{
	    		isValid = false;
	    		errorDataList.add(ErrorDataFactory.createErrorData(ErrorConstants.ASSERTION_GROUP_LIST_NULL, ErrorConstants.ERRORDOMAIN));
	    	}
	    	else if(!isAllWSDL)
	    	{
	    		isValid = false;
	    		errorDataList.add(ErrorDataFactory.createErrorData(ErrorConstants.NOT_ALL_WSDL, ErrorConstants.ERRORDOMAIN));
	    	}
	    	else
	    	{
	    		AssertionGroupAsset assertionGroupAsset = new AssertionGroupAsset();
	    		assertionGroupAsset.setAssetName(ConstantPopulator.getInstance().getValueOf(PropertyKeys.DEFAULT_WSDL_ASSERTION_GROUP_ASSET_NAME));
	    		...
	    		assertionGroupAsset.setVersion(ConstantPopulator.getInstance().getValueOf(PropertyKeys.DEFAULT_WSDL_ASSERTION_GROUP_ASSET_VERSION));
    ...
    private boolean validateAssertionGroupAsset(AssertionGroupAsset assertionGroupAssetReference, List<CommonErrorData> errorDataList)
    {
	boolean isValid = true;
	...
	assertionManager.setRepositoryServiceLocation(ConstantPopulator.getInstance().getValueOf(PropertyKeys.REPOSITORY_SERVICE_URL));
	...
	for (AssertionGroupAsset wsdlAssertionGroupAsset : wsdlAssertionGroupAssetList)
	{
	    ...
	    AssetReference groupAssetReference = new AssetReference(wsdlAssertionGroupAsset.getLibraryName(), wsdlAssertionGroupAsset.getAssetName(), wsdlAssertionGroupAsset.getVersion(),
		    AssertionGroupAssetTypes.ASSERTION_GROUP.value());
	    org.ebayopensource.turmeric.repositorymanager.assertions.AssertionGroup apiAssertionGroup = new AssetAssertionGroup(groupAssetReference);
	    ...
	    {
	    	//System.out.println("ASSETABLE ARTIFACT " + assertableArtifact.getArtifactAssetReference().getAssetName());
		try
		{
					...
					String userId = null;
					userId = messageContext.getRequestMessage()
							.getTransportHeader(SOAHeaders.AUTH_USERID);

					String password = null;
					password = messageContext.getRequestMessage()
							.getTransportHeader(SOAHeaders.AUTH_PASSWORD);
					//System.out.println("CALLING FRAMEWORK FUNCTIONS");
					apiAssertionReport = assertionManager.applyAssertionGroups(
							userId, password, apiAssertionGroupList,
							singleContentList);
					...
					wsdlAssertionGroupReport.getArtifactReport().add(
							wsdlArtifactReport);
				}
		catch(AssertionRuntimeException assertionRuntimeException)
		{
			...
			if(assertionRuntimeException.getErrorMessage()!= null)
	   		 ...
	   		 {
	   			 errorDataList.add(AssertionsServiceOperationUtil.parseGeneralExceptionMessage(new Exception(assertionRuntimeException.getThrowable())));
	   		 }
		}
   		 ...
   		 {
   			 errorDataList.add(AssertionsServiceOperationUtil.parseGeneralExceptionMessage(new Exception(assertionIllegalArgumentException.getThrowable())));
   		 }
   	 	}
