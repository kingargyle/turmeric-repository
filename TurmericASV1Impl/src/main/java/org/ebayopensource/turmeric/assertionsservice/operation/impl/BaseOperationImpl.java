...
public class BaseOperationImpl {
	...
	protected void convert(Object inputObject, Object outputObject)
			...
			if (inputObject instanceof AssertableArtifact) {
				...
				if (outputObject instanceof AssertableArtifactReference) {
					...
					if (assertableArtifact.getArtifactAssetReference() != null) {
						assertableArtifactReference
								.setArtifactAsset(assertableArtifact
										.getArtifactAssetReference());
						...
						convert(assertableArtifact.getArtifactExternal(),
								externalArtifactReference);
						assertableArtifactReference
								.setArtifactExternal(externalArtifactReference);
					}
				}
				...
				if (outputObject instanceof ExternalArtifactReference) {
					...
					externalArtifactReference
							.setContentName(ContentBindingName.FILE_PATH
									.value());
				}
				...
				if (outputObject instanceof ArtifactReport) {
					...
					for (AssertionResult assertionResult : apiAssertionReport
							.getAssertionResults()) {
						...
						if (assertionResult.getValidationResult().equals(
								ValidationResult.ERROR)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.ERROR);
						}
						if (assertionResult.getValidationResult().equals(
								ValidationResult.MUST)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.MUST);
						} else if (assertionResult.getValidationResult()
								.equals(ValidationResult.WARNING)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.WARNING);
						}
						if (assertionResult.getValidationResult().equals(
								ValidationResult.SHOULD)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.SHOULD);
						}
						if (assertionResult.getValidationResult().equals(
								ValidationResult.MAY)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.MAY);
						} else if (assertionResult.getValidationResult()
								.equals(ValidationResult.PASSED)) {
							wsdlAssertionResult
									.setAssertionSeverity(AssertionSeverity.WARNING);
						}
						wsdlArtifactReport.getAssertionResults().add(
								wsdlAssertionResult);
					}
				}
			}
	...
	protected boolean validateAssertion(Assertion assertionInput,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
		boolean foundContent = false;
		if (assertionInput.getExternalAssertion() != null) {
			foundContent = true;
			ExternalAssertion externalAssertion = assertionInput
					.getExternalAssertion();
			if (externalAssertion.getName() == null) {
				isValid = false;
				errorDataList.add(ErrorDataFactory.createErrorData(
						ErrorConstants.ASSERTION_NAME_MISSING,
						ErrorConstants.ERRORDOMAIN));
			}
			if (externalAssertion.getErrorSeverity() == null) {
				isValid = false;
				errorDataList.add(ErrorDataFactory.createErrorData(
						ErrorConstants.ASSERTION_ERROR_SEVERITY_MISSING,
						ErrorConstants.ERRORDOMAIN));
			}
			if (!validateExternalContent(
					externalAssertion.getAssertionScript(), errorDataList)) {
				isValid = false;
			}
			...
			if (externalAssertion.getAssertionModules() != null) {
				for (AssertionModule assertionModule : externalAssertion
						.getAssertionModules()) {
		...
		if (assertionInput.getAssertionAsset() != null) {
			if (foundContent) {
				errorDataList.add(ErrorDataFactory.createErrorData(
						ErrorConstants.MULTIPLE_CONTENT_TAG_POPULATION_WARNING,
						ErrorConstants.ERRORDOMAIN));
			}
			foundContent = true;
		...
		if (!foundContent) {
			isValid = false;
			errorDataList.add(ErrorDataFactory.createErrorData(
					ErrorConstants.ASSERTION_CONTENT_MISSING,
					ErrorConstants.ERRORDOMAIN));
		}
		return isValid;
	}

	protected boolean validateExternalContent(ExternalContent contentExternal,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
		boolean foundContent = false;
		...
		if (contentExternal.getBinaryContent() != null) {
			...
			if (contentExternal.getBinaryContent().getContent() == null
					|| contentExternal.getBinaryContent().getContent().length <= 0) {
				isValid = false;
				String[] params = new String[] { "a BinaryContent" };

				errorDataList.add(ErrorDataFactory.createErrorData(
						ErrorConstants.ASSERTION_OBJECT_CONTENT_MISSING,
	...
	protected boolean validateAssertionModule(AssertionModule assertionModule,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;

		if (assertionModule.getAssertionModuleExternal() != null) {
			if (!validateExternalAssertionModule(
					assertionModule.getAssertionModuleExternal(), errorDataList)) {
				isValid = false;
			}
		} else if (assertionModule.getModuleAssetReference() != null) {
			if (!validateAssertionModuleAsset(
					assertionModule.getModuleAssetReference(), errorDataList)) {
				isValid = false;
			}
		} else {
			isValid = false;
			errorDataList.add(ErrorDataFactory.createErrorData(
					ErrorConstants.MODULE_SCRIPTS_MISSING,
					ErrorConstants.ERRORDOMAIN));
		}
		return isValid;
	}

	protected boolean validateAssertionModuleAsset(
			AssertionModuleAsset assetReference,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
	...
	protected boolean validateAssetReference(AssetReference assetReference,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
		if (assetReference.getAssetName() == null) {
			isValid = false;
	...
	protected boolean validateArtifactAsset(ArtifactAsset artifactAsset,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
	...
	protected boolean validateExternalAssertionModule(
			ExternalAssertionModule externalAssertionModule,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
		...
		if (externalAssertionModule.getAssertionProcessorType() == null) {
			isValid = false;
			errorDataList.add(ErrorDataFactory.createErrorData(
					ErrorConstants.ASSERTION_PROCESSOR_TYPE,
					ErrorConstants.ERRORDOMAIN));
		}
		if (externalAssertionModule.getModuleScripts() != null
				&& externalAssertionModule.getModuleScripts().size() > 0) {
			for (ExternalContent contentExternal : externalAssertionModule
					.getModuleScripts()) {
	...
	protected boolean validateExternalArtifact(
		...
		if (externalArtifact.getBinaryContent() != null) {
			...
			foundContent = true;
			if (externalArtifact.getBinaryContent().getContent() == null
					|| externalArtifact.getBinaryContent().getContent().length <= 0) {
				isValid = false;
				String[] params = new String[] { "a BinaryContent" };
				errorDataList.add(ErrorDataFactory.createErrorData(
						ErrorConstants.ASSERTION_OBJECT_CONTENT_MISSING,
	...
	protected boolean validateAssertableArtifact(
			AssertableArtifact assertableArtifact,
			List<CommonErrorData> errorDataList) {
		boolean isValid = true;
		// boolean isWarning = false;
		boolean foundContent = false;
		...
		if (assertableArtifact.getArtifactAssetReference() != null) {
			foundContent = true;
			if (!validateArtifactAsset(
					assertableArtifact.getArtifactAssetReference(),
					errorDataList)) {
				isValid = false;
			}
		}
		if (assertableArtifact.getArtifactExternal() != null) {
			...
			if (!validateExternalArtifact(
					assertableArtifact.getArtifactExternal(), errorDataList)) {
				isValid = false;
			}
		}
		if (!foundContent) {
	...
	protected Object convert(Object inputObject) throws ObjectNotFoundException {
		...
		if (inputObject.getClass().equals(AssertableArtifact.class)) {
			...
			if (wsdlAssertableArtifact.getArtifactAssetReference() != null) {
				ArtifactAsset wsdlArtifactAssetReference = wsdlAssertableArtifact
						.getArtifactAssetReference();
				...
				if (wsdlArtifactAssetReference != null) {
					apiAssetReference = new org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference(
							wsdlArtifactAssetReference.getLibraryName(),
							wsdlArtifactAssetReference.getAssetName(),
							wsdlArtifactAssetReference.getVersion(),
							wsdlArtifactAssetReference.getAssetType());
				} else {
					new ObjectNotFoundException(
							"Reference to requeried Assertion Asset was not found");
				}

				AssetContent apiAssetContent = null;
				if (apiAssetReference != null) {
					apiAssetContent = new AssetContent("FILE_PATH",
							apiAssetReference, wsdlAssertableArtifact
									.getArtifactAssetReference()
									.getArtifactCategory().value());
				} else {
					new ObjectNotFoundException(
							"Reference to requeried Assertion Asset was not found");
				}
			...
			if (wsdlAssertableArtifact.getArtifactExternal() != null) {
				returnApiAssertionContent = (org.ebayopensource.turmeric.repositorymanager.assertions.AssertionContent) convert(wsdlAssertableArtifact
						.getArtifactExternal());
			}
			outputObject = returnApiAssertionContent;
			...
			org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference apiAssetReference = new org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference(
					assertionModuleAsset.getLibraryName(),
					assertionModuleAsset.getAssetName(),
					assertionModuleAsset.getVersion(), assertionModuleAsset
							.getAssetType().value());
			...
			org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference apiAssetReference = new org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference(
					wsdlAssetReference.getLibraryName(),
					wsdlAssetReference.getAssetName(),
					wsdlAssetReference.getVersion(), wsdlAssetReference
							.getAssetType().value());
			outputObject = apiAssetReference;
			...
			wsdlAssertionResultItem.setContentName(apiAssertionResultItem
					.getContentName());
			wsdlAssertionResultItem.setLineNumber(apiAssertionResultItem
					.getLineNumber());
			wsdlAssertionResultItem.setMessageText(apiAssertionResultItem
					.getMessageText());
			...
			wsdlAssertionResult.setAssertionDescription(apiAssertionResult
					.getAssertionDescription());
			wsdlAssertionResult.setAssertionName(apiAssertionResult
					.getAssertionName());
			wsdlAssertionResult.setAssertionVersion(apiAssertionResult
					.getAssertionVersion());

			if (!apiAssertionResult.getValidationResult().equals(
					ValidationResult.PASSED)) {
				wsdlAssertionResult
						.setValidationResult(org.ebayopensource.turmeric.repository.v1.services.ValidationResult.FAILED);
			} else {
				wsdlAssertionResult
						.setValidationResult(org.ebayopensource.turmeric.repository.v1.services.ValidationResult.PASSED);
			}

			if (apiAssertionResult.getResultItems() != null) {
				for (AssertionResultItem apiAssertionResultItem : apiAssertionResult
						.getResultItems()) {
					...
					wsdlAssertionResult.getResultItems().add(
							wsdlAssertionResultItem);
				}
			}
			...
			if (wsdlAssertionModule.getModuleAssetReference() != null) {
				...
				org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference apiAssetReference = (org.ebayopensource.turmeric.repositorymanager.assertions.AssetReference) convert(wsdlAssertionModule
						.getModuleAssetReference());
				assetContent = new AssetContent(
						apiAssetReference.getAssetName(), apiAssetReference,
						"Assertion Modelue Script");
				assertionContentList.add(assetContent);
				assertionModule = new BasicAssertionModule(
						apiAssetReference.getAssetName(),
						apiAssetReference.getVersion(),
						apiAssetReference.getAssetName(), assertionContentList,
						AssertionProcessor.XQUERY_TYPE);
			} else {
				ExternalAssertionModule assertionModuleExternal = wsdlAssertionModule
						.getAssertionModuleExternal();
				for (ExternalContent contentExternal : assertionModuleExternal
						.getModuleScripts()) {
				...
				assertionModule = new BasicAssertionModule(
						assertionModuleExternal.getName(),
						assertionModuleExternal.getVersion(),
						assertionModuleExternal.getDescription(),
						assertionContentList, assertionModuleExternal
								.getAssertionProcessorType().value());
			}
			...
			if (contentExternal.getBinaryContent() != null) {
				BinaryContent apiBinaryContent = new BinaryContent(
						ContentBindingName.FILE_PATH.value(),
						contentExternal.getBinaryContent().getContent());

				assertionContent = apiBinaryContent;
			}
			if (contentExternal.getStringContent() != null) {
				StringContent apiStringContent = new StringContent(
						ContentBindingName.FILE_PATH.value(),
						contentExternal.getStringContent().getContent());

				assertionContent = apiStringContent;
			}
			if (contentExternal.getUrlContent() != null) {
				assertionContent = new URLContent(
						ContentBindingName.FILE_PATH.value(),
						contentExternal.getUrlContent().getUrl());
			}
			...
			if (artifactExternal.getBinaryContent() != null) {
				BinaryContent apiBinaryContent = new BinaryContent(
						ContentBindingName.FILE_PATH.value(),
						artifactExternal.getBinaryContent().getContent());
			...
			if (artifactExternal.getTextContent() != null) {
				StringContent apiStringContent = new StringContent(
						ContentBindingName.FILE_PATH.value(),
						artifactExternal.getTextContent().getContent());
			...
			if (artifactExternal.getUrlContent() != null) {
				assertionContent = new URLContent(
						ContentBindingName.FILE_PATH.value(),
						artifactExternal.getUrlContent().getUrl());
			}
			outputObject = assertionContent;
		}
		return outputObject;
