...
public class AssetInfoCapsule implements AssetCapsuleInterface 
{
	 ...
	 * @param properties --> information required to connect to RS
	...
	public AssetInfoCapsule(Properties properties) throws IllegalArgumentException,AssertionRuntimeException 
	{
		...
		{
			throw new AssertionRuntimeException("Could not create client to Repository Service from withing AssetInfoCapsule due to following reasons: " + serviceException.getMessage(),serviceException);
		}
	}
	...
	public Object getAsset(String libraryName, String assetId) {
		...
		if(m_assetInfo == null) 
		{
	        ...
	        Library library = m_rsClient.getNamedLibrary(libraryName);
	        if (library == null)
	        {
	            throw new IllegalArgumentException("Could not find Library named '"+libraryName+"'");
	        }
