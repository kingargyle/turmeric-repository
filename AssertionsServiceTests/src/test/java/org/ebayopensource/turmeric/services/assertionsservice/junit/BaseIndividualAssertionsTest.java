...
public class BaseIndividualAssertionsTest extends BaseAssertionsServiceTest{
	...
	/**
	 * For an assertion "soa_sampleAssertionXXX_assertion", the sample WSDL to test the positive scenario need to be present as 
	 * "/WSDL/SampleAssertionXXX/Sample_Pass.wsdl". Similarly,  the sample WSDL to test the negative scenario need to be present as 
	 * "/WSDL/SampleAssertionXXX/Sample_Fail.wsdl"
	 */
	...
	protected AssertableArtifact createArtifactForPositiveScenario(String assertionAssetName){
		...
		artifactExternal.setBinaryContent(getBinaryContent(getTestArtifactWSDLFilepath(assertionAssetName) + "/Sample_Pass.wsdl"));
	...
	protected AssertableArtifact createArtifactForNegativeScenario(String assertionAssetName){
		...
		ExternalArtifact artifactExternal = new ExternalArtifact();
		artifactExternal.setContentType(ArtifactContentTypes.WSDL);
		artifactExternal.setBinaryContent(getBinaryContent(getTestArtifactWSDLFilepath(assertionAssetName) + "/Sample_Fail.wsdl"));
		artifact.setArtifactExternal(artifactExternal);
	...
	protected AssertableArtifact createArtifact(String assertionAssetName, String artifactFilename){
		...
		ExternalArtifact artifactExternal = new ExternalArtifact();
		artifactExternal.setContentType(ArtifactContentTypes.WSDL);
		artifactExternal.setBinaryContent(getBinaryContent(getTestArtifactWSDLFilepath(assertionAssetName) + "/" + artifactFilename));
		artifact.setArtifactExternal(artifactExternal);
